...
async function postRequest(data) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error || `API returned an error: ${response.status}`);
    }
    ...
    if (error.name === 'AbortError') {
      ...
      showAlert('Failed to post request ' + ollama_host + ' ' + error.message)
    }
...
async function populateModels() {
  ...
  try {
    ...
    if (data && data.models && Array.isArray(data.models)) {
      ...
      console.warn('No models found in the response or invalid data structure:', data);
      const option = document.createElement('option');
    ...
    chrome.storage.local.get('model', (result) => {
      ...
      if (result.model !== undefined && result.model !== '' && Array.from(selectElement.options).some(opt => opt.value === result.model)) {
      ...
      if (chatlogElement) {
        chatlogElement.classList.remove('spinner');
      }
      updateSettingString();
    ...
    showAlert(chrome.i18n.getMessage('unableToRead') + ': ' + error.message)
    const settings = document.getElementById('settings');
    if (settings) {
      settings.innerHTML = chrome.i18n.getMessage('modelsNotFound')
    }
...
const readPageButton = document.getElementById('read-page-btn');
...
if (readPageButton) {
  readPageButton.addEventListener('click', () => {
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      if (tabs[0].url.startsWith('chrome://') || tabs[0].url.startsWith('https://chrome.google.com/webstore')) {
        showAlert("Cannot access browser internal pages or Web Store.");
        return;
      }
      ...
      chrome.scripting.executeScript({
        ...
        if (chrome.runtime.lastError) {
          ...
          showAlert("Unable to read webpage. Please reload the page and try again.");
          return;
        }

        if (results && results[0] && results[0].result) {
          const text = results[0].result;
          // Show it in context area
          context_prompt.classList.remove('hidden');
          context_prompt.value = text;
          showAlert("Webpage content added as context!", "success");
          promptInput.focus();
        } else {
          showAlert("No text content found on this page.");
        }
      });
    });
  });
...
async function submitRequest() {
  ...
  updateChatlog(chatlog, contextInput, input, imageData);
...
docContent=""
  ...
  if (imageData) {
    const removeImage = imageContainer.querySelector('.remove-image');
  ...
  if (currentImageData) {
    data.images = [currentImageData.split(',')[1]];
  }
...
function handleImage(file) {
  ...
  reader.onload = (e) => {
    ...
    imageContainer.innerHTML = `
      <div class="bg-gray-500 rounded-lg sm:w-[35%] md:w-[30%] w-[50%] flex relative group fade-in mb-2">
        <div class="flex items-center justify-center p-2">
          <img src="${imageData}" alt="Preview" class="w-12 h-12 object-cover rounded mr-4">
          <div>
            <p class="text-white font-bold text-xs truncate">${file.name}</p>
            <p class="text-white opacity-75 text-xs">Image</p>
          </div>
        </div>
        <div class="absolute -top-2 -right-2 bg-red-200 hover:bg-red-400 p-1 rounded-full hidden group-hover:flex items-center justify-center remove-image cursor-pointer">
...
function updateChatlog(chatlog, contextInput, input, currentImageData) {
  const chatEntry = document.createElement('div');
  let imageHtml = '';
  if (currentImageData) {
    imageHtml = `<img src="${currentImageData}" class="max-w-[200px] rounded-lg mb-2 shadow-md">`;
  }
  ...
  <div class="absolute top-0 -left-10">
    ...
    chatEntry.innerHTML = `
      ...
      <div class="w-[95%]  flex justify-end items-start mb-4 fade-in group  countForEditing" id="${countForEditing}" >
        ...
        <div class="fade-in bg-gradient-to-r from-fuchsia-500 to-cyan-500 font-mediumtext-left text-white p-2 px-4 test-white rounded-bl-lg rounded-tr-lg text-lg rounded-tl-lg max-w-[90%] m-1 overflow-hidden">
          ${imageHtml}
          <p type="text" id="prompt" placeholder="Ask a follow-up" class="">
            ${input}
          </p>
        </div>
...
function showAlert(message, type = 'error') {
  ...
  // 기존 스타일 클래스 제거
  alertDiv.classList.remove('text-red-800', 'bg-red-50', 'dark:text-red-400', 
                           'text-green-800', 'bg-green-50', 'dark:text-green-400',
                           'bg-gray-800', 'border', 'border-green-500', 'border-red-500');
  
  // 공통 어두운 배경 및 테두리 스타일 추가
  alertDiv.classList.add('bg-gray-800', 'border');

  // 타입에 따른 밝은 글자색 및 테두리 색상 설정
  if (type === 'success') {
    alertDiv.classList.add('text-green-400', 'border-green-500');
  } else {
    alertDiv.classList.add('text-red-400', 'border-red-500');
  }
...
function updateSettingString() {
  settings.innerHTML = chrome.i18n.getMessage('runningOn') + ollama_host.split("//")[1];
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'TEXT_SELECTED') {
    const applyText = () => {
      ...
      if (inputField) {
        console.log('Setting prompt value to:', message.text);
        inputField.value = message.text.toString();
        inputField.focus();
