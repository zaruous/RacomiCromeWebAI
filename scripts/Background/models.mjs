...
async function getResponseFromLLM(query, port) {
  ...
  let selectedModel = await getModelFromStorage();
  ...
  if (!selectedModel) {
    try {
      ...
      if (tagsData.models && tagsData.models.length > 0) {
        ...
        throw new Error("No models found in Ollama. Please pull a model first.");
      }
    } catch (e) {
      port.postMessage({ type: 'ERROR', resp: 'No model selected and failed to fetch default: ' + e.message });
      return;
    }
  }
